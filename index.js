...
app.get('/account', (req, res) => {
  ...
  res.render('pages/account', 
    ...
    daily_challenges_completed: req.session.user[0].numChallengesCompleted,
    user_id: req.session.user[0].user_id,
    email: req.session.user[0].email,
    clash_tag: req.session.user[0].clash_tag,
    current_streak: req.session.user[0].current_streak,
    max_streak: req.session.user[0].max_streak,
    random_challenges_completed: req.session.user[0].random_challenges_completed
  });
});
...
function incrementBadUserChallengesCompleted(user_id) {
  // First, query the database for the current number of challenges completed
  db.any(`SELECT bad_challenges_completed FROM users WHERE user_id = ${user_id}`)
    .then(data => {
      //console.log("Incrementing Challenges")
      // With this result, add 1, and reinsert into the database
      let newCompleted = data[0].bad_challenges_completed + 1;
